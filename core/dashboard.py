...
def security_panel():
    ...
    table.add_row("🔎 Port Scans", "0 Completed")
    table.add_row("🌐 DNS Queries", "0 Completed")
    table.add_row("📄 Reports", "0 Generated")
    table.add_row("🛡 Threat Checks", "0 Completed")
    table.add_row("⚡ Toolkit", "🟢 Active")
...
def create_dashboard():
    ...
    layout["header"].update(
        Panel(
            Align.center(
                "[bold cyan]🛡 SentinelSOC Dashboard[/bold cyan]"
            ),
            border_style="cyan"
        )
    )
    ...
    layout["footer"].update(
        Panel(
            Align.center(
                "[bold green]Detect • Analyze • Defend | SentinelSOC v0.3[/bold green]"
            ),
            border_style="green"
        )
    )
